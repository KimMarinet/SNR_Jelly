...
const NAV_ITEMS: { id: Panel; label: string; icon: string; desc: string }[] = [
  {
    ...
    label: "파일 관리",
    icon: "AS",
    desc: "공용 이미지와 자료 파일을 업로드하고 카테고리별로 정리하는 구역입니다.",
  },
  {
    id: "board",
    label: "게시판 운영",
    icon: "BD",
    desc: "게시판 상태와 게시글 흐름을 한 화면에서 관리하는 구역입니다.",
  },
  {
    id: "landing-video",
    label: "랜딩 영상 관리",
    icon: "VD",
    desc: "메인 랜딩에 노출되는 배경 영상의 순서와 공개 여부를 제어합니다.",
  },
];
...
function Toast({
  ...
  return (
    <div
      className="admin-toast flex items-start gap-3 rounded-2xl px-4 py-3 shadow-xl backdrop-blur-sm animate-[toast-in_0.25s_ease-out]"
      data-error={item.error}
      style={{ minWidth: "260px", maxWidth: "360px" }}
    >
      <span
        className="mt-0.5 inline-flex shrink-0 rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-[0.14em]"
        style={
          item.error
            ? { backgroundColor: "var(--hub-danger-bg)", color: "var(--hub-danger-text)" }
            : { backgroundColor: "var(--hub-accent-soft)", color: "var(--hub-accent)" }
        }
      >
        {item.error ? "ERR" : "OK"}
      </span>
...
function DashboardStat({
  ...
  return (
    <article
      className={[
        "admin-stat-card rounded-2xl p-2.5",
        accent ? "admin-stat-card--accent" : "",
      ].join(" ")}
    >
      <p
        ...
        style={{
          color: accent
            ? "var(--hub-accent)"
            : "color-mix(in srgb, var(--hub-muted) 82%, transparent)",
        }}
      >
        {label}
      </p>
      <p
        className="mt-1 text-xl font-black tracking-tight [font-family:var(--font-space-grotesk),sans-serif] md:text-[1.35rem]"
        style={{
          color: accent
            ? "var(--hub-text)"
            : "color-mix(in srgb, var(--hub-text) 82%, transparent)",
        }}
      >
        {value}
      </p>
...
export function AdminControlPanel({
  ...
  async function handleHeroBackgroundUpload(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const response = await fetch("/api/admin/preferences/hero-background", {
        method: "POST",
        body: formData,
      });
  ...
  const totalAssets = initialActiveAssets.length;
  const trashedAssets = initialTrashedAssets.length;
  ...
  return (
    <>
      <div className="admin-shell">
        <header
          className={[
            "admin-hero admin-hero--compact admin-hero--embedded",
            heroBackgroundUrl ? "admin-hero--with-background" : "",
          ].join(" ")}
        >
          {heroBackgroundUrl ? (
            <>
              <div
                ...
                aria-hidden
              />
              <div className="admin-hero-background-overlay" aria-hidden />
            </>
          ) : null}
          ...
          <div className="admin-hero-body">
            <div className="admin-hero-bar">
              <div className="max-w-3xl">
                <span
                  className="inline-flex rounded-full border px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.18em] [font-family:var(--font-space-grotesk),sans-serif]"
                  style={{
                ...
                <div className="mt-3">
                  <h1
                    className="text-2xl font-black uppercase leading-none tracking-tight md:text-4xl [font-family:var(--font-space-grotesk),sans-serif]"
                    style={{ color: "var(--hub-text)" }}
                  >
                    Admin{" "}
                    <span
                      className="text-transparent"
                      style={{ WebkitTextStroke: "1px var(--hub-accent)" }}
                    >
                      Console
                    </span>
                  </h1>
                  <p
                    className="mt-2 max-w-2xl text-xs leading-relaxed md:text-sm"
                    style={{ color: "var(--hub-muted)" }}
                  >
                    관리자 화면은 운영 효율에 집중하도록 상단 영역을 축소했습니다. 현재 작업 구역을
                    빠르게 확인하고 바로 관리 작업으로 들어갈 수 있게 정리했습니다.
                  </p>
                </div>
              </div>
              ...
              <div className="admin-hero-links">
                ...
                <button
                  type="button"
                  onClick={openHeroBackgroundPicker}
                  className="admin-link-chip admin-link-chip--compact"
                  disabled={heroBackgroundUploading}
                >
                  {heroBackgroundUploading ? "배경 저장 중" : "개인 배경 설정"}
                </button>
                {heroBackgroundUrl ? (
                  <button
                    type="button"
                    onClick={handleHeroBackgroundRemove}
                    className="admin-link-chip admin-link-chip--compact"
                    disabled={heroBackgroundUploading}
                  >
                    배경 제거
                ...
                <Link href="/lounge" className="admin-link-chip admin-link-chip--compact">
                  라운지 보기
                </Link>
                <Link href="/" className="admin-link-chip admin-link-chip--compact">
                  랜딩 미리보기
                </Link>
              </div>
            </div>
            ...
            <div className="admin-hero-stats">
              <DashboardStat label="게시판 수" value={`${activeBoardCount + inactiveBoardCount}`} />
              <DashboardStat label="자산 파일" value={`${totalAssets}`} />
              <DashboardStat label="랜딩 영상" value={`${activeVideoCount}`} />
              <DashboardStat label="현재 작업" value={activeNav?.label ?? "--"} accent />
            </div>
          </div>
        </header>
        ...
        <div className="admin-dashboard admin-dashboard--merged">
          <aside className="admin-panel admin-sidebar">
            <div className="border-b pb-4" style={{ borderColor: "var(--hub-border)" }}>
              <p
              ...
              <p className="mt-2 text-lg font-bold" style={{ color: "var(--hub-text)" }}>
                작업 선택
              </p>
              <p className="mt-1 text-sm leading-relaxed" style={{ color: "var(--hub-muted)" }}>
                각 항목은 서로 다른 운영 구역입니다. 필요한 메뉴만 선택해 집중적으로 수정할 수
                있습니다.
              </p>
            </div>
            ...
            <nav className="space-y-2">
              {NAV_ITEMS.map((item) => {
                ...
                return (
                  ...
                  >
                    <span
                      className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl border text-xs font-black tracking-[0.14em] [font-family:var(--font-space-grotesk),sans-serif]"
                      style={{
                        borderColor: isActive
                          ? "color-mix(in srgb, var(--hub-border) 92%, transparent)"
                          : "var(--hub-border)",
                        backgroundColor: isActive
                          ? "color-mix(in srgb, var(--hub-surface-alt) 28%, transparent)"
                          : "transparent",
                        color: isActive ? "var(--hub-text)" : "var(--hub-muted)",
                      }}
                    >
                      {item.icon}
                    </span>
            ...
            <div
              className="mt-auto space-y-4 border-t pt-4"
              style={{ borderColor: "var(--hub-border)" }}
            >
              <div className="admin-stat-card rounded-2xl p-4">
                <p
                  className="text-[10px] font-semibold uppercase tracking-[0.16em] [font-family:var(--font-space-grotesk),sans-serif]"
                  style={{ color: "var(--hub-muted)" }}
                >
                ...
                <p className="mt-2 text-sm font-semibold" style={{ color: "var(--hub-text)" }}>
                  {activeNav?.label}
                </p>
              ...
              <button
                ...
                className="admin-link-chip w-full justify-center"
              >
                로그아웃
              </button>
            </div>
          ...
          <section className="admin-panel admin-workspace">
            <div className="admin-toolbar">
              <div>
                ...
                <h2
                  className="mt-2 text-2xl font-black tracking-tight [font-family:var(--font-space-grotesk),sans-serif]"
                  style={{ color: "var(--hub-text)" }}
                >
                  {activeNav?.label}
                </h2>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                >
                  게시판 {activeBoardCount + inactiveBoardCount}
                </span>
                <span
                  className="inline-flex rounded-full border px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.14em]"
                ...
                <span
                  ...
                  style={{ borderColor: "var(--hub-border)", color: "var(--hub-muted)" }}
                >
                  영상 {initialLandingVideos.length}
                </span>
              </div>
            </div>
